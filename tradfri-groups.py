...
def parse_args():
    ...
    parser.add_argument(
        "-g", "--groupid", help="get group ID from tradfri-status.py", required=True
    )
    parser.add_argument(
        "-v", "--value", help="power: on/off, brightness: 0 - 100", required=True
    )
...
def main():
    ...
    if args.action == "power":
        if args.value == "on" or args.value == "off":
            tradfriActions.tradfri_power_group(
                hubip, apiuser, apikey, args.groupid, args.value
            )
        else:
        ...
        if 0 <= int(args.value) <= 100:
            tradfriActions.tradfri_dim_group(
                hubip, apiuser, apikey, args.groupid, args.value
            )
        else:
