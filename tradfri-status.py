...
def main():
    ...
    for deviceid in tqdm(range(len(devices)), desc="Tradfri devices", unit=" devices"):
        lightbulb.append(
            tradfriStatus.tradfri_get_lightbulb(
                hubip, apiuser, apikey, str(devices[deviceid])
            )
        )
    ...
    for groupid in tqdm(range(len(groups)), desc="Tradfri groups", unit=" group"):
        lightgroup.append(
            tradfriStatus.tradfri_get_group(
                hubip, apiuser, apikey, str(groups[groupid])
            )
        )
    ...
    for _ in range(len(lightbulb)):
        try:
            ...
            try:
                ...
                warmth = round(
                    (warmth - 250) / (454 - 250) * 100, 1
                )  # reported as a percentage (100% maximum warmth)
            except KeyError:
                warmth = "NAN"

            if lightbulb[_]["3311"][0]["5850"] == 0:
                print(
                    "bulb ID {0:<5}, name: {1: <35}, brightness: {2: <3}, warmth: {3: >5}%, state: off".format(
                        lightbulb[_]["9003"], lightbulb[_]["9001"], brightness, warmth
                    )
                )
            else:
                print(
                    "bulb ID {0:<5}, name: {1: <35}, brightness: {2: <3}, warmth: {3: >5}%, state: on".format(
                        lightbulb[_]["9003"], lightbulb[_]["9001"], brightness, warmth
                    )
                )
        except KeyError:
    ...
    for _ in range(len(lightgroup)):
        if lightgroup[_]["5850"] == 0:
            print(
                "group ID: {0:<5}, name: {1: <16}, state: off".format(
                    lightgroup[_]["9003"], lightgroup[_]["9001"]
                )
            )
        else:
            print(
                "group ID: {0:<5}, name: {1: <16}, state: on".format(
                    lightgroup[_]["9003"], lightgroup[_]["9001"]
                )
            )
